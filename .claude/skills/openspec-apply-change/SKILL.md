...
description: 从 OpenSpec 变更中实现任务。当用户想要开始实现、继续实现或处理任务时使用。
license: MIT
...
**输入**：可选择指定变更名称。若未指定，检查是否能从对话上下文中推断。若模糊不清，必须提示用户选择可用的变更。
...
3. **获取 apply 指令**
   ...
   **处理各状态：**
   - 若 `state: "blocked"`（缺少 artifacts）：显示消息，建议使用 openspec-continue-change
